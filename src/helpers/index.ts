...
export const getConfigProperties = (): { [config: string]: unknown } => {
  return vscode.extensions.getExtension('florin-bratan.bravato-icons')
...
export const getThemeConfig = (section: string) => {
  return getConfig('bravato-icons').inspect(section);
};
...
export const setThemeConfig = (
  ...
  return getConfig('bravato-icons').update(section, value, global);
};
...
export const getExtensionPath = () =>
  vscode.extensions.getExtension('florin-bratan.bravato-icons')
    ?.extensionPath ?? '';

/** Get the configuration of the icons as JSON Object */
export const getBravatoIconsJSON = (): IconConfiguration => {
